...
public:
    ...
    bool start(const std::string& path) {
        ...
        if (server_fd_ < 0) {
            std::cerr << "[UDS Server] Socket 创建失败!" << std::endl;
            return false;
        }
        ...
        if (bind(server_fd_, (struct sockaddr*)&addr, sizeof(addr)) == -1) {
            std::cerr << "[UDS Server] Bind 失败! 路径: " << socket_path_ << std::endl;
            return false;
        }

        if (listen(server_fd_, 5) == -1) {
            std::cerr << "[UDS Server] Listen 失败!" << std::endl;
            return false;
        }

        setNonBlocking(server_fd_);
        // std::cout << "[UDS Server] 启动成功，等待算法进程接入..." << std::endl;
        return true;
    }
    ...
    bool acceptClient() {
        ...
        if (new_fd >= 0) {
            ...
            // std::cout << "[UDS Server] 算法进程已成功接入！" << std::endl;
            return true;
        }
        return false;
    }
    ...
    bool receiveData(T& data) {
        ...
        if (bytes_read == 0 || (bytes_read < 0 && errno != EAGAIN && errno != EWOULDBLOCK)) {
            // 返回 0 代表客户端断开连接
            // std::cout << "[UDS Server] 算法进程已断开连接。" << std::endl;
            close(client_fd_);
            client_fd_ = -1;
            return false;
        }
...
public:
    ...
    bool connectToServer(const std::string& path) {
        ...
        // std::cout << "[UDS Client] 成功连接至底层硬件代理！" << std::endl;
        return true;
    }
