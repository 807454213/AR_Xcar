...
static cv::Mat runCase(bool enabled, bool debugOverlay)
{
    ...
    config().tc.goldReachableWidthAddOuterLeft = 87;
    config().tc.goldReachableWidthAddOuterRight = 87;
...
int main()
{
    ...
    if (debug_off_changed != 0) {
        std::cerr << "debugOverlay=false should suppress gold band overlay\n";
        return 1;
    }
    if (debug_on_changed <= 0) {
        std::cerr << "debugOverlay=true should draw gold band pixels\n";
        return 2;
    }
    if (hud_white_pixels > 0) {
        std::cerr << "gold band overlay should not draw white HUD text\n";
        return 3;
    }
    return 0;
}
