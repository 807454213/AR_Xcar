...
static int read_image_stb(const char *path, image_buffer_t *image)
{
    ...
    if (c == 1)
    {
        ...
        for (int i = 0; i < w * h; i++)
        {
            rgb_data[i * 3] = pixeldata[i];
            rgb_data[i * 3 + 1] = pixeldata[i];
            rgb_data[i * 3 + 2] = pixeldata[i];
        }
        stbi_image_free(pixeldata);
        pixeldata = rgb_data;
        c = 3;
    ...
    else if (c == 4)
    {
        ...
        for (int i = 0; i < w * h; i++)
        {
            rgb_data[i * 3] = pixeldata[i * 4];
            rgb_data[i * 3 + 1] = pixeldata[i * 4 + 1];
            rgb_data[i * 3 + 2] = pixeldata[i * 4 + 2];
            // Ignore alpha
        }
        stbi_image_free(pixeldata);
        pixeldata = rgb_data;
