...
#include <set>
#include <vector>
// LABEL_NALE_TXT_PATH removed; label path passed into init_post_process

char *coco_cls_to_name(int cls_id)
{
    static char* my_labels[] = {
        (char*)"gold",
        (char*)"car",
        (char*)"human",
        (char*)"sign"
    ...
    int total_labels = sizeof(my_labels) / sizeof(my_labels[0]);

    if (cls_id >= 0 && cls_id < total_labels) {
        return my_labels[cls_id];
    }
...
{
    int lineCount = readLines(locationFilename, label, OBJ_CLASS_NUM);
    if (lineCount < 0)
        return -1;
    return lineCount;
}
...
static float deqnt_affine_to_f32(int8_t qnt, int32_t zp, float scale) {
    // Model outputs are already sigmoid-activated probabilities, no need for sigmoid
                      ...
                      float threshold)
{
    ...
    for (int i = 0; i < grid_h; i++)
    {
        for (int j = 0; j < grid_w; j++)
        {
            ...
            // Use score sum to quickly filter (使用浮点数比较)
            if (score_sum_tensor != nullptr)
            {
                float score_sum_f32 = deqnt_affine_u8_to_f32(score_sum_tensor[offset], score_sum_zp, score_sum_scale);
                if (score_sum_f32 < threshold)
                {
                    continue;
                }
            }

            // 初始化为第一个类别的分数 (使用浮点数比较)
            float max_score_f32 = deqnt_affine_u8_to_f32(score_tensor[offset], score_zp, score_scale);
            uint8_t max_score = score_tensor[offset];
            int max_class_id = 0;
            for (int c = 0; c < OBJ_CLASS_NUM; c++)
            {
                uint8_t cur_score = score_tensor[offset + c * grid_len];
                float cur_score_f32 = deqnt_affine_u8_to_f32(cur_score, score_zp, score_scale);
                if ((cur_score_f32 > threshold) && (cur_score_f32 > max_score_f32))
                {
                    max_score_f32 = cur_score_f32;
                    max_score = cur_score;
                    max_class_id = c;
                }
                offset += grid_len;
                      ...
                      float threshold)
{
    ...
    for (int i = 0; i < grid_h; i++)
    {
        for (int j = 0; j < grid_w; j++)
        {
            ...
            // 通过 score sum 起到快速过滤的作用 (使用浮点数比较)
            if (score_sum_tensor != nullptr)
            {
                float score_sum_f32 = deqnt_affine_to_f32(score_sum_tensor[offset], score_sum_zp, score_sum_scale);
                if (score_sum_f32 < threshold)
                {
                    continue;
                }
            ...
            // 初始化为第一个类别的分数 (使用浮点数比较)
            float max_score_f32 = deqnt_affine_to_f32(score_tensor[offset], score_zp, score_scale);
            int8_t max_score = score_tensor[offset];
            int max_class_id = 0;

            for (int c = 0; c < OBJ_CLASS_NUM; c++)
            {
                int8_t cur_score = score_tensor[offset + c * grid_len];
                float cur_score_f32 = deqnt_affine_to_f32(cur_score, score_zp, score_scale);
                if ((cur_score_f32 > threshold) && (cur_score_f32 > max_score_f32))
                {
                    max_score_f32 = cur_score_f32;
                    max_score = cur_score;
...
// 处理只有回归输出的模型（没有cls和obj头）
static int process_reg_only_i8(int8_t *box_tensor, int32_t box_zp, float box_scale,
                      int grid_h, int grid_w, int stride, int dfl_len,
                      std::vector<float> &boxes,
                      std::vector<float> &objProbs,
                      std::vector<int> &classId,
    ...
    for (int i = 0; i < grid_h; i++)
    {
        for (int j = 0; j < grid_w; j++)
        {
            ...
            if (w >= MIN_BOX_SIZE && h >= MIN_BOX_SIZE)
            {
                ...
                classId.push_back(0);  // 默认类别为0 (gold)
                validCount++;
            }
        }
    }
...
int post_process(rknn_app_context_t *app_ctx, rknn_output *outputs, resize_param_t *resize_param, float conf_threshold, float nms_threshold, object_detect_result_list *od_results)
{
    ...
    // 检查是否是纯回归输出模型（每个分支只有1个输出）
    bool is_reg_only = (app_ctx->io_num.n_output == 3);

    if (output_per_branch < 2 && !is_reg_only)
        return -1;
    ...
    if (is_reg_only)
    {
        for (int i = 0; i < 3; i++)
        {
            ...
            if (app_ctx->is_quant)
            {
                validCount += process_reg_only_i8((int8_t *)outputs[box_idx].buf, app_ctx->output_attrs[box_idx].zp, app_ctx->output_attrs[box_idx].scale,
                                          grid_h, grid_w, stride, dfl_len,
                                          filterBoxes, objProbs, classId, conf_threshold);
            }
            else
            {
                // FP32版本暂不支持，使用简化的i8处理
                validCount += process_reg_only_i8((int8_t *)outputs[box_idx].buf, app_ctx->output_attrs[box_idx].zp, app_ctx->output_attrs[box_idx].scale,
                                          grid_h, grid_w, stride, dfl_len,
                                          filterBoxes, objProbs, classId, conf_threshold);
            }
    ...
    // Apply PRE_NMS_TOPK
    if (validCount > PRE_NMS_TOPK)
    {
        std::vector<float> new_filterBoxes;
        std::vector<float> new_objProbs;
        std::vector<int> new_classId;
        for (int i = 0; i < PRE_NMS_TOPK; ++i)
        {
            int idx = indexArray[i];
            for (int j = 0; j < 4; ++j)
            {
                new_filterBoxes.push_back(filterBoxes[idx * 4 + j]);
            }
            new_objProbs.push_back(objProbs[i]);
            new_classId.push_back(classId[idx]);
        }
        filterBoxes = new_filterBoxes;
        objProbs = new_objProbs;
        classId = new_classId;
        validCount = PRE_NMS_TOPK;
    ...
    for (int i = 0; i < validCount; ++i)
    {
        if (indexArray[i] == -1 || last_count >= KEEP_TOPK)
        {
            continue;
        }
        int n = indexArray[i];
        ...
        od_results->results[last_count].box.left =
            (int)(clamp(x1, 0, model_in_w) / resize_param->scale_x);
        od_results->results[last_count].box.top =
            (int)(clamp(y1, 0, model_in_h) / resize_param->scale_y);
        od_results->results[last_count].box.right =
            (int)(clamp(x2, 0, model_in_w) / resize_param->scale_x);
        od_results->results[last_count].box.bottom =
            (int)(clamp(y2, 0, model_in_h) / resize_param->scale_y);
